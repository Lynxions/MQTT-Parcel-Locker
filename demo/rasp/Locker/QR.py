...
class QRCodeGeneratorApp:
    ...
    def generate_qr_code(self, order_id, otp):
        try:
            ...
            self.root.after(60*1000, self.clear_qr_code)

        except Exception as e:
            messagebox.showerror("Error", f"Failed to generate QR Code: {str(e)}")
    
    def display_sucess(self, cell_id):
        msg = f"Open Cell{cell_id}"
        self.clear_qr_code(msg)

    def clear_qr_code(self, msg="QR Code Deleted"):
        # Clear the QR code display from the canvas
        if self.canvas:
            self.canvas.delete("all")
            self.canvas.create_text(
                150, 150, text=msg, font=("Arial", 16), fill="red"
            )
        self.qr_image = None
    ...
    def display_qr_code(self):
        if self.qr_image is not None:
            ...
            #self.canvas.create_text(200, 50, text=msg, fill="black", font=("Arial", 16))
